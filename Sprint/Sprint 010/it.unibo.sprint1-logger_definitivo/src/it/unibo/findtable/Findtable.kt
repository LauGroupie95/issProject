		...
		return { //this:ActionBasciFsm
				...
				state("stepDone") { //this:State
					action { //it:State
						...
						if((NumPassi==mapMaxX-4)){ forward("endRow", "endRow" ,"findtable" ) 
						 }
						else
				...
				state("tableFound") { //this:State
					action { //it:State
						...
						fineScansioneTavolo = true
						 ...
						 solve("assert(table('${posizioneX}','${posizioneY}'))","") //set resVar	
				...
				state("exploreEdge") { //this:State
					action { //it:State
						if(fineScansioneTavolo == true){ forward("endScanningTable", "endScanningTable" ,"findtable" ) 
						 }
						else
						 { StepAroundTable++
				...
				state("goHome") { //this:State
					action { //it:State
						...
						var posizioneX = itunibo.planner.plannerUtil.getPosX()
								var posizioneY = itunibo.planner.plannerUtil.getPosY()
						solve("assert(robot('${posizioneX}','${posizioneY}','${itunibo.planner.moveUtils.getDirection(myself)}'))","") //set resVar	
						solve("robot(X,Y,D)","") //set resVar	
						if(currentSolution.isSuccess()) { println("Invio a navi la posizione corrente del robot: ${getCurSol("X")} ${getCurSol("Y")} ${getCurSol("D")}")
						emit("robotPosition", "robotPosition(${getCurSol("X")},${getCurSol("Y")},${getCurSol("D")})" ) 
						delay(1000) 
						 }
						forward("endScanningTable", "endScanningTable" ,"master1" ) 
					}
